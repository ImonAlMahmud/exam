...
try {
    ...
    $sql = "
        SELECT 
            ...
            e.description, 
            e.is_omr_exam,
            (SELECT MAX(oak.id) FROM omr_answer_keys oak WHERE oak.exam_id = e.id) AS answer_key_id, -- New: Fetch the latest answer_key_id
        ...
        GROUP BY es.id -- Group by schedule ID to prevent duplicates from multiple answer keys
